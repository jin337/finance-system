...
import { Button, Checkbox, DatePicker, Image, Input, Layout, Menu, Message, Modal, Space, Table } from '@arco-design/web-react'
import {
  IconApps,
  IconCalendar,
  IconDelete,
...
const Report = () => {
  ...
  const openPreview = (record) => {
    if (['pdf', '.pdf', 'xlsx', 'docx'].includes(record.fileext)) {
    } else {
      setVisible(true)
    }
  }
      ...
      <Layout>
        <Layout.Header className='flex items-center justify-between border-b border-neutral-200 px-5 py-4'>
          {selectList.length > 0 ? (
            ...
            <Space size='large'>
              <Button shape='round' type='primary' icon={<IconScan />}>
                扫描
              </Button>
              <Button shape='round' type='primary' icon={<IconUpload />}>
                上传
              </Button>
              <Button
                shape='round'
              ...
              <Button shape='round' type='outline' icon={<IconExport />}>
                生成财务报表
              </Button>
            </Space>
          )}
